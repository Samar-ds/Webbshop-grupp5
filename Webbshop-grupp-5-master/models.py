...
class Product(db.Model):
    ...
    CampaignPrice = db.Column(db.Integer, unique=False, nullable=False)
...
class ProductCart(db.Model):
    ...
    product_id = db.Column(db.Integer(), db.ForeignKey('Products.ProductID', ondelete='CASCADE'))
...
class Cart(db.Model):
    ...
    Products = db.relationship('Product', secondary='ProductCart')

class ProductWishlist(db.Model):
    __tablename__ = "ProductWishlist"
    Id = db.Column(db.Integer(), primary_key=True)
...
class Wishlist(db.Model):
    ...
    Id = db.Column(db.Integer, primary_key=True)
    UserID = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
    Products = db.relationship('Product', secondary='ProductWishlist')
...
class ProductRating(db.Model):
    ...
    Point_1 = db.Column(db.Integer, unique=False, nullable=False)
    Point_2 = db.Column(db.Integer, unique=False, nullable=False)
    Point_3 = db.Column(db.Integer, unique=False, nullable=False)
    Point_4 = db.Column(db.Integer, unique=False, nullable=False)
    Point_5 = db.Column(db.Integer, unique=False, nullable=False)
    Point_total = db.Column(db.Integer, unique=False, nullable=False)
    Point_count = db.Column(db.Integer, unique=False, nullable=False)
    Point_avarage = db.Column(db.Float, unique=False, nullable=False)

class Mails(db.Model):
    __tablename__="MailAdresser"
    EpostID = db.Column(db.Integer, primary_key=True)
    Epostadress = db.Column(db.String(40), unique=True,nullable=False)
    newsletters = db.relationship('Newsletter', secondary='newsletter_mails')
    IsActive = db.Column(db.Boolean, unique=False, nullable=False)

class Newsletter(db.Model):
    __tablename__ = "Newsletter"
    NewsletterId = db.Column(db.Integer, primary_key=True)
    Title = db.Column(db.String(40), unique=False, nullable=False)
    Content = db.Column(db.String(1000), unique=False, nullable=False)
    HasBeenSent = db.Column(db.Boolean, unique=False, nullable=False)

class NewsletterMails(db.Model):
    Id = db.Column(db.Integer, primary_key=True)
    Mail_id = db.Column(db.Integer, db.ForeignKey('MailAdresser.EpostID', ondelete='CASCADE'))
    Newsletter_id = db.Column(db.Integer, db.ForeignKey('Newsletter.NewsletterId', ondelete='CASCADE'))
...
def addProduct(db,namn,supplierid, categoryid, quantityperunit,unitprice,unitsinstock,unitsonorder,reorderlevel,discontinued):
    ...
    if a == None:
        ...
        db.session.commit()
        c = Product.query.filter_by(ProductName=namn).first()
        new = ProductRating()
        new.Product_id = c.ProductID
        new.Point_1 = 0 
        new.Point_2 = 0
        new.Point_3 = 0
        new.Point_4 = 0
